...
# --- ESCUDO ANTI-ERRORES DE LIBRERÍAS ---
try:
    import smtplib
...
# --- LIMPIADOR EXTREMO DE TEXTOS PARA CORREO ---
def limpiar_texto_ascii(texto):
    """Fuerza cualquier texto a ser 100% compatible con los servidores de correo (ASCII)"""
    texto_str = str(texto)
    # Reemplazo manual de la maldita ñ que causa el 99% de los problemas
    texto_str = texto_str.replace('ñ', 'n').replace('Ñ', 'N')
...
def enviar_correo_completo(destinatario, nombre_cliente, pdf_bytes, xlsx_cot_bytes, xlsx_ctrl_bytes, filename):
    ...
    try:
        # Aplicamos la limpieza extrema a todo lo que toca el servidor de correo
        safe_filename = limpiar_texto_ascii(filename).replace(" ", "_")
        safe_cliente = limpiar_texto_ascii(nombre_cliente)
        
        msg = EmailMessage()
        msg['Subject'] = f"Nueva Cotizacion: {safe_filename}"
        msg['From'] = st.secrets["gmail"]["user"]
        msg['To'] = destinatario
        
        cuerpo = f"Se ha generado la documentacion para la cotizacion {safe_filename}.\nCliente: {safe_cliente}.\n\nSe adjuntan los 3 archivos."
        msg.set_content(cuerpo, charset='utf-8')
        
        # Adjuntar archivos con nombres 100% limpios
        msg.add_attachment(pdf_bytes, maintype='application', subtype='pdf', filename=f"{safe_filename}.pdf")
        msg.add_attachment(xlsx_cot_bytes, maintype='application', subtype='vnd.openxmlformats-officedocument.spreadsheetml.sheet', filename=f"{safe_filename}_Comercial.xlsx")
        msg.add_attachment(xlsx_ctrl_bytes, maintype='application', subtype='vnd.openxmlformats-officedocument.spreadsheetml.sheet', filename=f"{safe_filename}_Control.xlsx")
