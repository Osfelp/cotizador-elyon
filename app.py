...
# --- CLASE PDF (Con Salto de Página Automático) ---
class PDF(FPDF):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Activa el salto de página automático a 15mm del final de la hoja para no cortar tablas
...
def generar_excel_estilizado(df_data, es_control=False):
    ...
    if not df_data.empty:
        for cat in df_data["Categoría"].unique():
            ...
            # AGREGAMOS "PRECIO VENTA UNIT." EN EL EXCEL DE CONTROL
            if es_control:
                headers = ["DESCRIPCIÓN", "DIAS", "CANT.", "COSTO UNIT.", "COSTO TOTAL", "PRECIO VENTA UNIT.", "PRECIO VENTA", "GANANCIA", "% GANANCIA"]
            else:
            ...
            for _, row in subset.iterrows():
                ...
                if es_control:
                    ...
                    ct = d * c * cu
                    vu = float(row["Precio Unitario"]) # Precio Venta Unitario (Con incremento)
                    vt = float(row["Subtotal Item"])   # Precio Venta Total
                    gan = vt - ct
                ...
                for idx, v in enumerate(vals, 1):
                    ...
                    # Identificamos la columna de Porcentaje para aplicar los colores
            ...
            # Fila de Subtotales (Para Cuadro de Control)
            if es_control:
                ...
                for idx, v in enumerate(vals_sub, 1):
                    ...
                    if v != "":
                        ...
                        if idx in [5, 7, 8]:  # Costo Total, Precio Venta, Ganancia
                            cell.number_format = currency_format
                        elif idx == 9:        # % Ganancia
                            cell.number_format = '0.0%'
                            if v >= 0.35:
                ...
                total_costo_general += sub_costo
                total_venta_general += sub_venta
                total_ganancia_general += sub_gan
            curr_row += 1

    # Fila de Totales Finales (Para Cuadro de Control)
    if es_control:
    ...
    # Fila de Totales Finales (Para Cotización Comercial)
    if not es_control:
...
def generar_pdf():
    ...
    if logo_b:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as t:
            t.write(logo_b)
            t_p = t.name
        pdf.image(t_p, x=10, y=10, w=60)
        os.remove(t_p)

    pdf.set_font("Arial", 'B', 16)
    pdf.cell(190, 10, "COTIZACIÓN COMERCIAL", ln=True, align="R")
    
    pdf.set_font("Arial", '', 10)
    pdf.cell(190, 5, f"Referencia: {cotizacion_num}", ln=True, align="R")
    pdf.cell(190, 5, f"Fecha: {fecha_emision.strftime('%d/%m/%Y')}", ln=True, align="R")
    pdf.ln(15)
    ...
    pdf.set_font("Arial", '', 10)
    pdf.cell(100, 8, f" Cliente: {solicitante_empresa}", border='LT')
    ...
    for cat in df_validos["Categoría"].unique():
        ...
        for _, r in sc.iterrows():
            pdf.cell(75, 8, str(r["Descripción"]), 1)
            pdf.cell(15, 8, str(int(r["Días"])), 1, 0, 'C')
            pdf.cell(20, 8, str(int(r["Cantidad"])), 1, 0, 'C')
            pdf.cell(40, 8, formato_moneda(r['Precio Unitario']), 1, 0, 'R')
