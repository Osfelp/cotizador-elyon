...
# Configuración de columnas (incluye la imagen disimulada)
config_columnas = {
    "📸": st.column_config.ImageColumn("📸", width="small", help="Pega el link web de la imagen"),
...
def generar_excel_estilizado(df_data, es_control=False):
    ...
    if not df_data.empty:
        for cat in df_data["Categoría"].unique():
            ...
            headers = ["DESCRIPCIÓN", "DIAS", "CANT.", "COSTO UNIT.", "COSTO TOTAL", "PRECIO VENTA", "GANANCIA", "LINK IMAGEN"] if es_control else ["DESCRIPCIÓN", "DIAS", "CANT.", "PRECIO UNITARIO", "SUBTOTAL"]
            ...
            for _, row in subset.iterrows():
                ...
                img_link = str(row.get("📸", ""))
                
                if es_control:
                    cu = float(row["Costo Unitario"]); ct = d * c * cu; vt = float(row["Subtotal Item"])
                    vals = [row["Descripción"], d, c, cu, ct, vt, vt - ct, img_link]
                else:
