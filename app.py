...
try:
    ...
    from email.header import Header # <--- ¡Nueva herramienta para soportar tildes y Ñ!
...
# --- AUTOMATIZACIÓN PROTEGIDA Y CON SOPORTE PARA 'Ñ' Y TILDES ---
def enviar_correo_completo(destinatario, nombre_cliente, pdf_bytes, xlsx_cot_bytes, xlsx_ctrl_bytes, filename):
    ...
    try:
        # Limpiamos el nombre del archivo para que los sistemas de correo no molesten
        safe_filename = filename.replace('ñ','n').replace('Ñ','N').replace('á','a').replace('é','e').replace('í','i').replace('ó','o').replace('ú','u')
        
        msg = MIMEMultipart()
        msg['From'] = st.secrets["gmail"]["user"]
        msg['To'] = destinatario
        
        # Usamos Header con utf-8 para que el ASUNTO soporte eñes y tildes sin explotar
        msg['Subject'] = Header(f"Cotización: {filename}", 'utf-8')
        
        cuerpo = f"Se ha generado la documentación completa para la cotización {filename}.\nCliente: {nombre_cliente}.\n\nSe adjuntan los 3 archivos solicitados."
        # Especificamos utf-8 en el cuerpo del correo
        msg.attach(MIMEText(cuerpo, 'plain', 'utf-8'))
        
        # 1. Adjuntar PDF
        p1 = MIMEBase('application', 'octet-stream')
        p1.set_payload(pdf_bytes); encoders.encode_base64(p1)
        p1.add_header('Content-Disposition', 'attachment', filename=f"{safe_filename}.pdf")
        msg.attach(p1)

        # 2. Adjuntar Excel Comercial
        p2 = MIMEBase('application', 'octet-stream')
        p2.set_payload(xlsx_cot_bytes); encoders.encode_base64(p2)
        p2.add_header('Content-Disposition', 'attachment', filename=f"{safe_filename}_Comercial.xlsx")
        msg.attach(p2)

        # 3. Adjuntar Excel de Control
        p3 = MIMEBase('application', 'octet-stream')
        p3.set_payload(xlsx_ctrl_bytes); encoders.encode_base64(p3)
        p3.add_header('Content-Disposition', 'attachment', filename=f"{safe_filename}_Control.xlsx")
        msg.attach(p3)
