...
with col_l:
    try:
        st.image(URL_LOGO_WEB, width=280)
    except:
        st.write("ELYON")
with col_t:
    st.title("Cotización")
...
# Ajuste de columnas: Más espacio para datos base, casillas pequeñas para IVA/FEE y el Checkbox al final
...
# Checkbox "Sin Fee" (alineado visualmente hacia abajo para que empate con las casillas)
with c7:
    st.markdown("<div style='margin-top: 32px;'></div>", unsafe_allow_html=True)
    sin_fee = st.checkbox("Sin Fee", key="sin_fee_chk")

# Lógica del Fee: Si está marcado "Sin Fee", se bloquea la casilla y queda en 0.
...
# --- MOTOR DE PDF ---
def generar_pdf(num, fecha_em, ciu, cli, cli_cont, cli_mail, cli_tel, evento_ref, evento_f, evento_l, evento_as, df_completo, sub, fee, fee_pct, iva, iva_pct, total):
    # Generamos el PDF en TAMAÑO CARTA (Letter)
    ...
    try:
        req_wm = requests.get(URL_WATERMARK_PDF)
        if req_wm.status_code == 200:
            ...
            # Marca de agua ajustada al tamaño carta (w=145, centrada)
            pdf.image(tmp_wm_path, x=35, y=85, w=145)
    except:
        pass

    try:
        req_logo = requests.get(URL_LOGO_PDF)
        if req_logo.status_code == 200:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp_logo:
                tmp_logo.write(req_logo.content)
                tmp_logo_path = tmp_logo.name
            # Logo a color un poco más grande (w=60 en lugar de 45)
    ...
    pdf.cell(150, 8, f"FEE {fee_pct}%", 1, 0, 'R', fill=True)
    pdf.cell(40, 8, formato_moneda(fee), 1, 1, 'R')

    pdf.cell(150, 8, f"IVA {iva_pct}%", 1, 0, 'R', fill=True)
    ...
    pdf.ln(10)
    pdf.set_text_color(0, 0, 0)
    # Tamaño de letra más pequeño (tamaño 6) para la letra chica de los términos
...
if not df_global.empty and not df_global["Descripción"].str.strip().eq("").all():
    ...
    with col_btn2:
        st.download_button(
            label="📊 Descargar Cotización (Sheets/Excel)",
            data=csv_cot_data,
    ...
    with col_btn3:
        st.download_button(
            label="📈 Descargar Cuadro Control (Sheets/Excel)",
            data=csv_ctrl_data,
            ...
            if not solicitante_empresa or not solicitante_contacto:
                ...
                with st.spinner("Generando archivo con marca de agua..."):
                    ...
                    ruta_pdf = generar_pdf(
                        ...
                        df_global, subtotal_neto, fee_total, int(fee_pct_final), iva_total, int(iva_porcentaje), total_general
                    )
