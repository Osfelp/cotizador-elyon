...
st.markdown(f"""
    ...
    div[data-testid="column"]:nth-of-type(1) [data-testid="stDownloadButton"] > button:hover {{
        ...
        background: linear-gradient(135deg, #006829 0%, #008f39 100%) !important;
    }}

    /* 2. COT EXCEL (Naranja Ocre) */
    ...
    div[data-testid="column"]:nth-of-type(2) [data-testid="stDownloadButton"] > button:hover {{
        transform: translateY(-3px) !important; box-shadow: 0 6px 12px rgba(211, 84, 0, 0.4) !important;
        background: linear-gradient(135deg, #e67e22 0%, #d35400 100%) !important;
    }}

    /* 3. CUADRO CONTROL (Azul Acero) */
    ...
    div[data-testid="column"]:nth-of-type(3) [data-testid="stDownloadButton"] > button:hover {{
        ...
        background: linear-gradient(135deg, #1f618d 0%, #2980b9 100%) !important;
    }}
...
class PDF(FPDF):
    def footer(self):
        ...
        # Se elimina la firma del PDF para mantenerlo netamente comercial
...
def generar_pdf():
    ...
    if wm_bytes:
        ...
        # Marca de agua reducida (80) y perfectamente centrada
        pdf.image(tmp_wm_path, x=68, y=80, w=80)
    ...
    # Todos los anchos fijos a 190 mm exactos para alineación perfecta de tablas
    ...
    for cat in cat_ordenadas:
        if cat in categorias_usadas:
            ...
            # Título de sección forzado a 190mm
            pdf.cell(190, 8, f" SECCIÓN: {cat.upper()}", ln=True, fill=True, border=1)
            
            pdf.set_fill_color(30, 30, 30) 
            pdf.set_text_color(255, 255, 255)
            pdf.set_font("Arial", 'B', 8)
            # La suma de las celdas da 190 (75+15+20+40+40)
...
if listo_para_descarga:
    ...
    def generar_csv_estructurado(df_data, es_control=False):
        ...
        if es_control:
            # Cuadro Control con nuevas métricas financieras
            ...
            if not df_data.empty:
                ...
                for cat in cat_ordenadas:
                    if cat in categorias:
                        ...
                        writer.writerow(["DESCRIPCIÓN", "DIAS", "CANT.", "COSTO UNIT.", "COSTO TOTAL", "PRECIO VENTA", "GANANCIA"])
                        ...
                        for _, row in subset.iterrows():
                            ...
                            unit_costo = int(round(row["Costo Unitario"]))
                            item_costo_total = int(round(dias * cant * unit_costo))
                            
                            item_venta_total = int(row["Subtotal Item"])
                            item_ganancia = item_venta_total - item_costo_total
                            
                            sub_costo += item_costo_total
                            sub_venta += item_venta_total
                            sub_ganancia += item_ganancia
                            
                            writer.writerow([desc, dias, cant, unit_costo, item_costo_total, item_venta_total, item_ganancia])
                        
                        total_costo_general += sub_costo
                        total_venta_general += sub_venta
                        total_ganancia_general += sub_ganancia
                        
                        writer.writerow(["", "", "", f"Subtotal {cat}:", sub_costo, sub_venta, sub_ganancia])
                        writer.writerow([])
                
                writer.writerow(["", "", "", "TOTALES GENERALES", int(total_costo_general), int(total_venta_general), int(total_ganancia_general)])
        else:
...
# --- FIRMA INVISIBLE EN LA PLATAFORMA ---
