...
# --- GENERADOR PDF ---
def generar_pdf():
    # --- ESCUDO PROTECTOR PARA TEXTOS RAROS ---
    def limpiar(texto):
        if pd.isna(texto) or texto is None: return ""
        # Reemplaza cualquier emoji o símbolo incompatible con PDF por un carácter seguro
    ...
    pdf.set_font("Arial", '', 10); pdf.cell(190, 5, f"Referencia: {limpiar(cotizacion_num)}", ln=True, align="R"); pdf.ln(15)
    ...
    pdf.cell(100, 8, f" Cliente: {limpiar(solicitante_empresa)}", border='LT'); pdf.cell(90, 8, f" Contacto: {limpiar(solicitante_contacto)}", border='RT', ln=True)
    pdf.cell(190, 8, f" Ciudad: {limpiar(ciudad)}  |  Email: {limpiar(solicitante_email)}", border='LR', ln=True)
    pdf.cell(190, 8, f" Teléfono: {limpiar(solicitante_telefono)}", border='LRB', ln=True); pdf.ln(5)
    
    for cat in st.session_state.cat_order:
        subset = df_validos[df_validos["Categoría"] == cat] if not df_validos.empty else pd.DataFrame()
        if not subset.empty:
            pdf.ln(8); pdf.set_fill_color(230, 230, 230); pdf.set_font("Arial", 'B', 10); pdf.cell(190, 8, f" SECCIÓN: {limpiar(cat).upper()}", ln=True, fill=True, border=1)
            ...
            for _, r in subset.iterrows():
                desc = limpiar(str(r["Descripción"]))
                lineas_totales = 0
            ...
            pdf.set_font("Arial", 'B', 9); pdf.cell(150, 8, f"Subtotal {limpiar(cat)}:", 1, 0, 'R'); pdf.cell(40, 8, formato_moneda(redondear_al_mil(subset['Subtotal Item'].sum())), 1, 1, 'R')
    ...
    pdf.multi_cell(0, 3, limpiar(texto_terminos))
