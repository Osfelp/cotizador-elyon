...
def generar_pdf():
    ...
    pdf.ln(10); if pdf.get_y() + 30 > 260: pdf.add_page()
...
# Lógica del botón con automatización (Si no hay configurado Secrets, no pasa nada)
if col_b1.download_button("📥 COT PDF", pdf_data, f"Cot_{cotizacion_num}_{empresa_nombre}.pdf", "application/pdf", disabled=not listo_para_descargar, use_container_width=True):
    if "gmail" in st.secrets or "google_drive" in st.secrets:
        info_asesor = ASESORES[asesor_selec]
        enviar_correo_automatico(info_asesor["email"], solicitante_empresa, pdf_data, xlsx_ctrl, f"Cot_{cotizacion_num}_{empresa_nombre}")
        subir_a_drive(pdf_data, f"Cot_{cotizacion_num}_{empresa_nombre}.pdf", info_asesor["drive_folder"], "application/pdf")
        st.toast(f"✅ Copia enviada y respaldada en la nube por {asesor_selec}")
