...
# --- II. GESTIÓN POR CATEGORÍAS (Motor de Memoria Corregido) ---
st.markdown("<hr>", unsafe_allow_html=True)
...
if 'edited_categorias' not in st.session_state:
...
def guardar_edicion(old_name, idx):
    ...
    if new_name and new_name != old_name and new_name not in st.session_state.categorias:
        # BAKEO DE DATOS: Pasamos las ediciones hechas al nuevo nombre para no perderlas
    ...
    with col_del:
        if cat_name != "General":
            ...
            if st.button("🗑️", key=f"del_{cat_name}"):
                ...
                st.session_state.edited_categorias.pop(cat_name, None)
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
    
    # Tabla de datos - SE SEPARA EL ORIGEN DEL DESTINO PARA EVITAR EL RESETEO DE CELDAS
    df_base = st.session_state.categorias.get(cat_name, pd.DataFrame([DEFAULT_VALS]))
    edited_df = st.data_editor(df_base, num_rows="dynamic", use_container_width=True, hide_index=True, key=f"ed_{cat_name}", column_config=config_columnas)
    
    # Guardamos los resultados editados en un diccionario separado (No sobreescribimos la base)
