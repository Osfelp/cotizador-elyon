...
try:
    import smtplib
    from email.message import EmailMessage # <--- NUEVO MOTOR MODERNO DE CORREOS (Adiós a los errores ASCII)
...
# --- AUTOMATIZACIÓN BLINDADA CONTRA ERRORES DE IDIOMA ---
def enviar_correo_completo(destinatario, nombre_cliente, pdf_bytes, xlsx_cot_bytes, xlsx_ctrl_bytes, filename):
    ...
    try:
        # Limpiador de nombres de archivo (quitar tildes, eñes y caracteres raros para evitar rechazos del servidor)
        safe_filename = unicodedata.normalize('NFKD', filename).encode('ASCII', 'ignore').decode('utf-8')
        safe_filename = re.sub(r'[^\w\s-]', '', safe_filename).strip().replace(" ", "_")
        
        # Motor moderno EmailMessage
        msg = EmailMessage()
        msg['Subject'] = f"Cotización: {filename}" # El asunto sí soporta acentos
