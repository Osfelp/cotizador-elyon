...
st.markdown(f"""
    ...
    [data-testid="stDataEditor"] {{ background-color: #1e1f20; border: 1px solid #444746; border-radius: 10px; overflow: hidden; }}
    ...
    [data-testid="stDownloadButton"] > button:hover, .stButton>button:hover {{ 
        ...
        box-shadow: 0 6px 12px rgba(0, 143, 57, 0.4) !important;
    ...
    .btn-delete>button {{ background: #ff4b4b !important; box-shadow: none !important; }}
    ...
    div[data-testid="metric-container"] {{
        background-color: #1e1f20; border: 1px solid #444746;
...
def generar_pdf():
    ...
    # CREACIÓN SEGURA DEL ARCHIVO TEMPORAL
    temp_pdf = tempfile.NamedTemporaryFile(delete=False, suffix=".pdf")
    temp_pdf_path = temp_pdf.name
    temp_pdf.close() # Clave para evitar el WinError 32
...
with col_btn3:
    st.download_button(
        label="📈 Descargar Cuadro control",
        data=csv_ctrl_data,
