...
# --- CLASE PDF (CON SALTO DE PÁGINA ARREGLADO) ---
class PDF(FPDF):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Habilita el salto de página automático a 15mm del final de la hoja
        self.set_auto_page_break(auto=True, margin=15)
        
    def footer(self):
        # Esta función solo se usa si quisieras poner números de página.
        # Por ahora la dejamos vacía para mantener el diseño limpio.
        pass
...
def agregar_nueva_seccion():
    nueva_cat = st.session_state.nueva_cat_input.strip()
    if nueva_cat and nueva_cat not in st.session_state.categorias:
        st.session_state.categorias[nueva_cat] = pd.DataFrame([DEFAULT_VALS])
    st.session_state.nueva_cat_input = ""

col_nueva1, col_nueva2 = st.columns([10, 1])
col_nueva1.text_input("Añadir nueva sección...", label_visibility="collapsed", placeholder="Nombre de la nueva sección...", key="nueva_cat_input", on_change=agregar_nueva_seccion)
col_nueva2.button("➕", on_click=agregar_nueva_seccion)
...
def generar_excel_estilizado(df_data, es_control=False):
    ...
    if not df_data.empty:
        for cat in df_data["Categoría"].unique():
            ws.cell(curr_row, 1, f"SECCIÓN: {cat.upper()}").font = Font(bold=True)
            curr_row += 1
            
            headers = ["DESCRIPCIÓN", "DIAS", "CANT.", "COSTO UNIT.", "COSTO TOTAL", "PRECIO VENTA", "GANANCIA", "% GANANCIA"] if es_control else ["DESCRIPCIÓN", "DIAS", "CANT.", "PRECIO UNITARIO", "SUBTOTAL"]
            ...
            for _, row in subset.iterrows():
                ...
                if es_control:
                    cu = float(row["Costo Unitario"]); ct = d * c * cu; vt = float(row["Subtotal Item"])
                    gan = vt - ct; pct = (gan / vt) if vt > 0 else 0
                    vals = [row["Descripción"], d, c, cu, ct, vt, gan, pct]
            ...
            if es_control:
                ...
                for idx, v in enumerate(vals_sub, 1):
                    ...
                    if v != "":
                        ...
                        if idx in [5, 6, 7]: cell.number_format = currency_format
                        elif idx == 8:
                            cell.number_format = '0.0%'
...
def generar_pdf():
    ...
    # 1. Agregamos el LOGO y el título (solo en la primera hoja)
    logo_b = obtener_imagen_bytes(URL_LOGO_PDF)
    if logo_b:
    ...
    # 2. Agregamos la MARCA DE AGUA (solo en la primera hoja para no saturar si son varias)
    wm_b = obtener_imagen_bytes(URL_WATERMARK_PDF)
    if wm_b:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as t: t.write(wm_b); t_p = t.name
        # Marca de agua en Y=100 para que quede detrás del cuadro principal
        pdf.image(t_p, x=68, y=100, w=80); os.remove(t_p)

    # 3. Dibujamos los datos del cliente
    ...
    # 4. Dibujamos las tablas de ítems
    # multi_cell permite que la descripción sea larga y baje renglón si es necesario
    for cat in df_validos["Categoría"].unique():
        ...
        for _, r in sc.iterrows():
            # Guardar posición X, Y actual
            x_start = pdf.get_x()
            y_start = pdf.get_y()
            
            desc_text = str(r["Descripción"])
            # Multi_cell dibuja la descripción y calcula cuántos renglones usó
            pdf.multi_cell(75, 6, desc_text, 1, 'L')
            y_end = pdf.get_y()
            altura_fila = y_end - y_start
            
            # Volvemos a subir para dibujar el resto de las columnas con la misma altura
            pdf.set_xy(x_start + 75, y_start)
            pdf.cell(15, altura_fila, str(int(r["Días"])), 1, 0, 'C')
            pdf.cell(20, altura_fila, str(int(r["Cantidad"])), 1, 0, 'C')
            pdf.cell(40, altura_fila, formato_moneda(r['Precio Unitario']), 1, 0, 'R')
            pdf.cell(40, altura_fila, formato_moneda(r['Subtotal Item']), 1, 1, 'R')
            
            # Nos aseguramos de estar abajo para la siguiente fila
            pdf.set_y(y_end)
    ...
    # 5. Dibujamos los totales finales
    ...
    # 6. Dibujamos Términos y Condiciones
    pdf.ln(10); pdf.set_text_color(100, 100, 100); pdf.set_font("Arial", 'B', 6)
