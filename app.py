...
st.markdown(f"""
    ...
    /* Botones de gestión (Pequeños) */
    .btn-manage>button {{ height: auto !important; background-color: #3a3b3c !important; color: white !important; padding: 5px !important; margin-top: 28px; }}
    .btn-manage>button:hover {{ background-color: #555555 !important; }}
    .btn-delete>button {{ background-color: #ff4b4b !important; color: white !important; margin-top: 28px; }}
...
# --- II. GESTIÓN POR CATEGORÍAS (Ordenable y Renombrable) ---
st.markdown("<hr>", unsafe_allow_html=True)
...
# Iteramos sobre el orden guardado
for i, cat_name in enumerate(list(st.session_state.cat_order)):
    c_title, c_up, c_down, c_del = st.columns([7, 1, 1, 1])
    
    # Campo editable para el título de la sección
    nuevo_nombre = c_title.text_input(f"Título de la sección {i+1}", value=cat_name, key=f"title_{i}")
    
    # Si el usuario cambió el nombre
    if nuevo_nombre != cat_name and nuevo_nombre not in st.session_state.categorias:
        st.session_state.categorias[nuevo_nombre] = st.session_state.categorias.pop(cat_name)
        st.session_state.cat_order[i] = nuevo_nombre
        st.rerun()

    c_up.markdown('<div class="btn-manage">', unsafe_allow_html=True)
    if c_up.button("⬆️ Subir", key=f"up_{i}"):
        if i > 0:
            st.session_state.cat_order[i], st.session_state.cat_order[i-1] = st.session_state.cat_order[i-1], st.session_state.cat_order[i]
            st.rerun()
    c_up.markdown('</div>', unsafe_allow_html=True)

    c_down.markdown('<div class="btn-manage">', unsafe_allow_html=True)
    if c_down.button("⬇️ Bajar", key=f"down_{i}"):
        if i < len(st.session_state.cat_order) - 1:
            st.session_state.cat_order[i], st.session_state.cat_order[i+1] = st.session_state.cat_order[i+1], st.session_state.cat_order[i]
            st.rerun()
    c_down.markdown('</div>', unsafe_allow_html=True)

    c_del.markdown('<div class="btn-delete">', unsafe_allow_html=True)
    ...
    # Tabla de datos
    df_cat = st.session_state.categorias[nuevo_nombre]
    edited_df = st.data_editor(df_cat, num_rows="dynamic", use_container_width=True, hide_index=True, key=f"ed_{nuevo_nombre}", column_config=config_columnas)
    st.session_state.categorias[nuevo_nombre] = edited_df # Guardar estado al editar
    ...
    df_calc["Categoría"] = nuevo_nombre 
...
def generar_pdf():
    ...
    if wm_b:
        ...
        pdf.image(t_p, x=68, y=100, w=80); os.remove(t_p) # Bajamos la marca de agua al centro (Y=100)
    if logo_b:
    ...
    for cat in st.session_state.cat_order:
        ...
        for _, r in sc.iterrows():
            ...
            # Cálculo exacto de líneas para que no se desborde el texto
            ancho_columna = 73.0
            ...
            altura_fila = max(8, lineas_estimadas * 5) # Mínimo 8mm de alto, si es largo, 5mm por línea
            
            # Verificamos si la fila cabe en la hoja, si no, salta de página (279.4mm alto - 20mm margen = 259)
            ...
            # Dibujamos la descripción multilinea
            pdf.multi_cell(75, 5 if lineas_estimadas > 1 else 8, desc, 1, 'L')
            y_end = pdf.get_y() # Donde terminó de escribir el texto
            
            altura_real = max(8, y_end - y_start) # La altura final es lo que ocupó el texto
            
            # Posicionamos las demás celdas a la derecha
            ...
            # Aseguramos que la siguiente fila inicie abajo de la más grande
    ...
    # Comprobación de salto de página antes de los totales
    ...
    # Comprobación de salto de página antes de términos
