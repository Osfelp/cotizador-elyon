...
def generar_excel_estilizado(df_data, es_control=False):
    ...
    # Formato de Información General en Excel (con formato de número y correo)
    ws.cell(curr_row, 1, "INFORMACIÓN GENERAL").font = Font(bold=True)
    curr_row += 1
    ws.cell(curr_row, 1, "Cliente/Empresa:").font = Font(bold=True)
    ws.cell(curr_row, 2, solicitante_empresa)
    ws.cell(curr_row, 3, "Contacto:").font = Font(bold=True)
    ws.cell(curr_row, 4, solicitante_contacto)
    curr_row += 1
    
    ws.cell(curr_row, 1, "Ciudad:").font = Font(bold=True)
    ws.cell(curr_row, 2, ciudad)
    ...
    try:
        # Intenta guardar el teléfono como número real
        ...
        for cat in df_data["Categoría"].unique():
            ...
            for _, row in subset.iterrows():
                ...
                for idx, v in enumerate(vals, 1):
                    ...
                    if any(x in headers[idx-1] for x in ["UNIT", "TOTAL", "VENTA", "GANANCIA", "SUBTOTAL", "PRECIO"]):
                        cell.number_format = currency_format
                    elif headers[idx-1] == "% GANANCIA":
            ...
            if es_control:
                ...
                vals_sub = ["", "", "", f"Subtotales:", sub_costo, sub_venta, sub_gan, sub_pct]
...
def generar_pdf():
    ...
    for cat in df_validos["Categoría"].unique():
        ...
        for _, r in sc.iterrows():
            pdf.cell(75, 8, str(r["Descripción"]), 1); pdf.cell(15, 8, str(int(r["Días"])), 1, 0, 'C'); pdf.cell(20, 8, str(int(r["Cantidad"])), 1, 0, 'C'); pdf.cell(40, 8, formato_moneda(r['Precio Unitario']), 1, 0, 'R'); pdf.cell(40, 8, formato_moneda(r['Subtotal Item']), 1, 1, 'R')
