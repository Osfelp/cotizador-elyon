...
st.sidebar.info("Sube un archivo de Excel (.xlsx) generado previamente por esta plataforma para continuar editándolo o recuperarlo.")
...
# --- GENERADOR PDF CON SOLUCIÓN DE ALTURA Y BORDES PERFECTOS ---
def generar_pdf():
    ...
    for cat in st.session_state.cat_order:
        ...
        if not subset.empty:
            ...
            for _, r in subset.iterrows():
                ...
                # Calcular lineas exactas detectando los "Enters" (\n) manuales
                lineas_totales = 0
                ...
                # Si no cabe, salta de página
                if pdf.get_y() + altura_estimada > 260: 
                    pdf.add_page()
                
                x_start, y_start = pdf.get_x(), pdf.get_y()
                
                # Escribe el texto SIN bordes primero para que se ajuste libremente
                pdf.multi_cell(75, h_linea, desc, border=0, align='L')
                altura_real = max(8, pdf.get_y() - y_start)
                
                # Dibuja el marco perfecto alrededor del texto
                pdf.rect(x_start, y_start, 75, altura_real)
                
                # Dibuja el resto de las celdas de la fila usando la altura exacta
                ...
                # Asegura que la siguiente fila inicie bien abajo
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 💾 GUARDAR PROGRESO")
st.sidebar.info("¿Llevas mucho tiempo editando? Descarga este borrador rápidamente para asegurar tus datos contra apagones.")
