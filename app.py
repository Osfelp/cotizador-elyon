...
st.markdown(f"""
    ...
    /* Botones de Descarga con Colores Específicos */
...
# Definimos los valores por defecto que pediste
...
if col_nueva2.button("➕"):
    if nueva_cat and nueva_cat not in st.session_state.categorias:
        # Al crear sección nueva, insertamos la fila por defecto
        st.session_state.categorias[nueva_cat] = pd.DataFrame([DEFAULT_VALS])
        st.rerun()

df_global = pd.DataFrame()

# Configuración de las columnas para el editor (aquí se definen los valores por defecto al pulsar "+")
config_columnas = {
...
for cat_name in list(st.session_state.categorias.keys()):
    ...
    # Aplicamos la configuración de columnas para que el botón "+" use los nuevos defaults
...
def generar_excel_estilizado(df_data, es_control=False):
    ...
    if not df_data.empty:
        for cat in df_data["Categoría"].unique():
            ...
            headers = ["DESCRIPCIÓN", "DIAS", "CANT.", "COSTO UNIT.", "COSTO TOTAL", "PRECIO VENTA", "GANANCIA"] if es_control else ["DESCRIPCIÓN", "DIAS", "CANT.", "PRECIO UNITARIO", "SUBTOTAL"]
            ...
            for _, row in subset.iterrows():
                ...
                if es_control:
                    ...
                    vals = [row["Descripción"], d, c, cu, ct, vt, vt - ct]
                else:
...
if listo:
    ...
    col_b3.download_button("📈 EXPORTAR CUADRO CONTROL", xlsx_ctrl, f"Control_{cotizacion_num}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
else:
