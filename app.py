...
# --- SISTEMA DE RECARGA DE PROYECTO DESDE EXCEL ---
if "uploaded_filename" not in st.session_state:
    st.session_state.uploaded_filename = ""

st.sidebar.markdown("### 🔄 RECARGAR COTIZACIÓN")
st.sidebar.info("Sube un archivo de Excel (.xlsx) generado previamente por esta plataforma para continuar editándolo.")
uploaded_file = st.sidebar.file_uploader("Subir Excel", type=["xlsx"])

if uploaded_file is not None:
    if uploaded_file.name != st.session_state.uploaded_filename:
        try:
            # Leer Información General del Backup
            ...
            # Limpiar datos 'nan' de celdas vacías
            for k in ['val_ciu', 'val_emp', 'val_con', 'val_ema', 'val_tel', 'val_ref', 'val_lug', 'val_asi']:
                if st.session_state[k] == 'nan': st.session_state[k] = ''

            # Leer Tablas del Backup
            df_raw = pd.read_excel(uploaded_file, sheet_name="ELYON_BACKUP", skiprows=19)
            
            st.session_state.cat_order = []
            st.session_state.categorias = {}
            st.session_state.edited_categorias = {}
            
            for cat in df_raw['Categoría'].dropna().unique():
                st.session_state.cat_order.append(cat)
                cat_df = df_raw[df_raw['Categoría'] == cat][["Descripción", "Días", "Cantidad", "Costo Unitario", "Incremento (%)"]].copy()
                st.session_state.categorias[cat] = cat_df
            
            if not st.session_state.cat_order:
                st.session_state.cat_order = ["General"]
                st.session_state.categorias = {"General": pd.DataFrame([DEFAULT_VALS])}
            
            st.session_state.uploaded_filename = uploaded_file.name
            st.sidebar.success("✅ Cotización cargada con éxito!")
            st.rerun()

        except Exception as e:
            st.sidebar.error("❌ Error al leer el archivo. Asegúrate de que sea un Excel generado por esta plataforma.")
...
config_columnas = {
    ...
    "Días": st.column_config.NumberColumn("Días", width="small", min_value=0.0, step=1.0),
    "Cantidad": st.column_config.NumberColumn("Cant.", width="small", min_value=0.0, step=1.0),
    "Costo Unitario": st.column_config.NumberColumn("Costo Unitario ($)", width="medium", step=1.0), 
    "Incremento (%)": st.column_config.NumberColumn("Incr. (%)", width="small", step=1.0)
}
...
# --- GENERADOR EXCEL CON MEMORIA INCRUSTADA ---
def generar_excel_estilizado(df_data, es_control=False):
    ...
    curr_row = 1
    ws.cell(curr_row, 1, "CUADRO DE CONTROL INTERNO" if es_control else "COTIZACIÓN COMERCIAL").font = Font(bold=True, size=14); curr_row += 1
    ws.cell(curr_row, 1, f"Evento: {evento_referencia}"); curr_row += 1
    ws.cell(curr_row, 1, f"Fecha Emisión: {fecha_emision.strftime('%d/%m/%Y')}"); curr_row += 2
    
    ws.cell(curr_row, 1, "INFORMACIÓN GENERAL").font = Font(bold=True); curr_row += 1
    ws.cell(curr_row, 1, "Cliente/Empresa:").font = Font(bold=True); ws.cell(curr_row, 2, solicitante_empresa)
    ws.cell(curr_row, 3, "Contacto:").font = Font(bold=True); ws.cell(curr_row, 4, solicitante_contacto); curr_row += 1
    ...
    if solicitante_email: cell_email.hyperlink = f"mailto:{solicitante_email}"; cell_email.font = Font(color="0000FF", underline="single")
    curr_row += 1
    
    ws.cell(curr_row, 1, "Teléfono:").font = Font(bold=True)
    try: cell_tel = ws.cell(curr_row, 2, int(solicitante_telefono.replace(" ", "").replace("+", ""))); cell_tel.number_format = '0'
    except: ws.cell(curr_row, 2, solicitante_telefono)
    curr_row += 2
    ...
    # === HOJA OCULTA DE RESPALDO (MEMORIA DE PROYECTO) ===
    ws_raw = wb.create_sheet("ELYON_BACKUP")
    ws_raw.sheet_state = 'hidden'
    
    datos_generales = {
        "cotizacion_num": cotizacion_num, "ciudad": ciudad, "fecha_emision": fecha_emision.strftime('%Y-%m-%d'),
        "validez_dias": validez_dias, "sin_fee": sin_fee, "fee_porcentaje_input": fee_porcentaje_input,
        "iva_porcentaje": iva_porcentaje, "solicitante_empresa": solicitante_empresa, "solicitante_contacto": solicitante_contacto,
        "solicitante_email": solicitante_email, "solicitante_telefono": solicitante_telefono, "evento_referencia": evento_referencia,
        "evento_fecha": evento_fecha.strftime('%Y-%m-%d'), "evento_lugar": evento_lugar, "evento_asistentes": evento_asistentes
    }
    ...
    if not df_data.empty:
        headers_raw = list(df_data.columns)
        for col_idx, col_name in enumerate(headers_raw, 1):
            ws_raw.cell(row=20, column=col_idx, value=col_name)
...
def generar_pdf():
    ...
    for cat in st.session_state.cat_order:
        ...
        for _, r in sc.iterrows():
            ...
            lineas_estimadas = max(1, math.ceil(ancho_texto / 73.0))
            altura_fila = max(8, lineas_estimadas * 5)
            
            if pdf.get_y() + altura_fila > 255: pdf.add_page()
            ...
            pdf.cell(15, altura_real, str(int(r["Días"])), 1, 0, 'C'); pdf.cell(20, altura_real, str(int(r["Cantidad"])), 1, 0, 'C')
            pdf.cell(40, altura_real, formato_moneda(r['Precio Unitario']), 1, 0, 'R'); pdf.cell(40, altura_real, formato_moneda(r['Subtotal Item']), 1, 1, 'R')
            pdf.set_y(y_start + altura_real)

        pdf.set_font("Arial", 'B', 9); pdf.cell(150, 8, f"Subtotal {cat}:", 1, 0, 'R'); pdf.cell(40, 8, formato_moneda(redondear_al_mil(sc['Subtotal Item'].sum())), 1, 1, 'R')
...
# --- IV. EXPORTAR DOCUMENTOS ---
st.markdown("<hr>", unsafe_allow_html=True)
st.markdown("### IV. EXPORTAR DOCUMENTOS")

listo_para_descargar = not df_validos.empty
if not listo_para_descargar: st.info("⚠️ Agrega al menos un ítem a la tabla para habilitar las descargas.")
...
col_b1.download_button("📥 COT PDF", pdf_data, f"{filename_base}.pdf", "application/pdf", disabled=not listo_para_descargar, use_container_width=True)
